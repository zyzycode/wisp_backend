...
Отдельный Python 3.10+ сервис FastAPI. Все файлы и окружение находятся в `backend_proxy`.

## Запуск в Windows (PowerShell)

На текущей машине окружение и библиотеки уже установлены. Откройте PowerShell
и выполните:

```powershell
cd C:\CODE\project_wisp\backend_proxy
...
Активация окружения не требуется. Остановка: Ctrl+C.

При первой установке на другой машине: установите Python 3.10+, перейдите
в папку `backend_proxy`, создайте `.env` по `.env.example` и укажите свой ключ.
Затем выполните:
